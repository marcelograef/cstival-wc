...
import { getData, saveRange } from '../../../services/api/api-requests.js';
import { getRealPositionLong, getRealPositionROL } from '../../../utils/position.js';
import { calculateAvg } from '../../../utils/calculations/calculateInfo.js';
import Button from '../../common/Button';
import CardTable from '../../features/PokerTable';
import Field from '../../common/Field';
import InfoContainer from '../../features/InfoContainer';
import './index.scss';
import '../PushPositionStack/index.scss';
...
export const LoadRange = () => {
	...
	}, [situation, setTableValues, effectiveStack]);
				...
				<PositionSelector
					...
					onClick={() => {
						...
						if (realYourPos && realVillainPos) {
							getData('ROR', `${realYourPos}|${realVillainPos}`, effectiveStack).then(rangeData => {
								setRange(rangeData);
								setTableValues(rangeData);
							});
						}
			...
			return (
				<PositionSelector
					...
					onClick={() => {
						...
						if (realYourPos && realVillainPos) {
							getData('RES3', `${realYourPos}|${realVillainPos}`, effectiveStack).then(rangeData => {
								setRange(rangeData);
								setTableValues(rangeData);
							});
						}
		...
		return positionsArray.map(p => (
			<PositionSelector
				...
				onClick={() => {
					...
					getData('ROL', `${realYourPos}`, effectiveStack).then(rangeData => {
						setRange(rangeData);
						setTableValues(rangeData);
					});
	...
	const loadRangePush = (rangeSelector, index) => {
		...
		setSituation(sitAux);
		getData('PUSH', sitAux,effectiveStack).then(rangeData => {
			setRange(rangeData);
			setTableValues(rangeData);
		});
	};
	...
	}, [positionSelected, bbsSelected, setTableValues, effectiveStack]);

	// Add new useEffect for effectiveStack changes
	useEffect(() => {
		if (!situation) return;

		setTableValues(initialState);
		getData(key, situation, effectiveStack).then(rangeData => {
			setRange(rangeData);
			setTableValues(rangeData);
		});
	}, [effectiveStack, key, situation, setTableValues]);
